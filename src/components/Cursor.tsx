 ...
 * The ring expands when hovering anything interactive — DOM (a/button/input)
 * or 3D objects (drei's useCursor sets body.style.cursor = 'pointer').
 * Ink-on-porcelain palette for the light theme.
 */
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      domHover = !!(e.target as Element | null)?.closest?.('a, button, input, [data-cursor]')
    }
